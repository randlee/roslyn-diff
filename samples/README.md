...
**View comprehensive HTML samples:**
```bash
open html-samples/tree-document.html
```

**Regenerate all HTML samples:**
```bash
./generate-html-samples.sh
```

See [html-samples/README.md](html-samples/README.md) for detailed documentation of all HTML output modes.

## Samples Overview

### html-samples (NEW!)

**Location:** `samples/html-samples/`

**Purpose:** Comprehensive showcase of ALL HTML output modes supported by roslyn-diff.

**Generated by:** `generate-html-samples.sh` script (fully reproducible)

**Samples Include:**
- **Basic Modes**: Document vs Fragment mode
- **View Modes**: Tree vs Inline view with various context levels
- **Additional Files**: UserService comparison demonstrating complex file structures
- **Impact Classification**: All impact levels and filtering options
- **Multi-TFM**: Target framework moniker analysis samples
- **Whitespace Handling**: All whitespace modes
- **Combinations**: Complex feature combinations

**Key Features:**
- 25+ comprehensive HTML samples covering every output mode
- Regenerable via single script command
- Detailed README with usage guide and comparisons
- Real code examples using actual project files

**Use Cases:**
- Learning all HTML output capabilities
- Choosing the right output mode for your needs
- Integration examples for web apps and dashboards
- Reference for command-line options

**Documentation:** See [html-samples/README.md](html-samples/README.md)
